...
import { registerForLive } from "@/lib/register.functions";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Vibe Coding Live — NLD Creative × Pluss.dev" },
      {
        name: "description",
        content:
          "بث مباشر مجاني ليلة الخميس عبر Google Meet. ابنِ أسرع، بِع أذكى، وبرمج بِـ Vibe — مع عبد الصمد ومهدي.",
      },
    ],
  }),
...
function ScrollProgress() {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 top-0 z-[60] h-px bg-line/40">
      <div
        ref={ref}
        className="h-full origin-right bg-live transition-transform duration-100 ease-out"
...
function Corners() {
  // Fixed viewfinder brackets — broadcast feel
  const base =
    "pointer-events-none fixed z-[55] h-5 w-5 border-ink/80";
  return (
    <>
      ...
      {/* Timestamp/REC indicator bottom-left of viewport */}
      <div
        dir="ltr"
    ...
    <header className="sticky top-0 z-40 border-b border-line bg-paper/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6 sm:px-10">
        ...
        <button
          ...
          className="group inline-flex items-center gap-2 border border-ink px-4 py-2 text-[12px] font-medium tracking-wider transition-colors hover:bg-ink hover:text-paper"
        >
  ...
  return (
    <section className="grain relative overflow-hidden">
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-4rem)] max-w-5xl flex-col items-center justify-center px-6 py-24 text-center sm:px-10">
        ...
        <h1
          className="fade-up mt-10 text-balance text-5xl font-extralight leading-[1.15] tracking-[-0.01em] text-ink sm:text-6xl md:text-7xl lg:text-[5.25rem]"
        ...
        >
          ...
          بِع أذكى.
          <br />
          <span className="font-light italic">برمج بِـ Vibe.</span>
        </h1>
        ...
        <div className="fade-up mt-12" style={{ animationDelay: "360ms" }}>
          <button
            onClick={scrollToForm}
            className="group inline-flex items-center gap-3 bg-ink px-7 py-4 text-[12px] font-medium tracking-[0.18em] text-paper transition-colors hover:bg-paper hover:text-ink hover:outline hover:outline-1 hover:outline-ink"
          >
        ...
        <div
          className="fade-up mt-24 flex flex-col items-center gap-4 text-[12px] tracking-[0.18em] text-ink-muted sm:flex-row sm:gap-6"
...
function SectionLabel({ children, ltr = false }: { children: React.ReactNode; ltr?: boolean }) {
  ...
  const cls = ltr
    ...
    : `${base} text-xs font-medium tracking-normal`;
  return (
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-32 sm:px-10 sm:py-40">
        ...
        <ul className="border-t border-line">
          {TOPICS.map((item, i) => (
            ...
            >
              <span className="font-mono text-sm text-ink-muted sm:text-base">{item.n}</span>
              <h3 className="text-balance text-2xl font-extralight tracking-[-0.005em] text-ink sm:text-4xl md:text-5xl">
                {item.t}
              </h3>
    ...
    <section className="mx-auto max-w-6xl px-6 py-32 sm:px-10 sm:py-40">
      ...
      <div className="mt-16 grid grid-cols-1 gap-16 sm:grid-cols-2 sm:gap-12">
        {hosts.map((h, i) => (
          ...
          >
            <div className="h-24 w-24 rounded-full border border-line bg-paper-soft" aria-hidden />
            <div>
...
function RegisterForm() {
  const submit = useServerFn(registerForLive);
  ...
  return (
    <section id="register" className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-3xl px-6 py-32 sm:px-10 sm:py-40">
        <div className="reveal text-center">
          <SectionLabel>احجز مقعدك</SectionLabel>
          <h2 className="mt-8 text-balance text-4xl font-extralight leading-[1.2] tracking-[-0.01em] text-ink sm:text-5xl">
            مجاني للانضمام. المقاعد محدودة.
          </h2>
          <p className="mx-auto mt-6 max-w-md text-base font-light leading-loose text-ink-soft">
            اترك معلوماتك وسنرسل لك رابط Google Meet قبل بداية البث.
          </p>
        </div>

        <div className="reveal mt-16" style={{ transitionDelay: "120ms" }}>
          {status === "success" ? (
            <div className="border border-line bg-paper/60 p-12 text-center backdrop-blur-sm">
              <p className="font-mono text-[11px] uppercase tracking-[0.3em] text-ink-muted">مؤكَّد</p>
            ...
            <form ref={formRef} onSubmit={onSubmit} className="space-y-8">
              ...
              {error && (
                <p className="font-mono text-[11px] tracking-[0.18em] text-ink">{error}</p>
              )}

              <button
                type="submit"
                disabled={status === "loading"}
                className="group inline-flex w-full items-center justify-center gap-3 bg-ink px-7 py-5 text-[12px] font-medium tracking-[0.18em] text-paper transition-colors hover:bg-paper hover:text-ink hover:outline hover:outline-1 hover:outline-ink disabled:opacity-50"
              >
...
function Field({
  ...
  return (
    <label className="block">
      <span className="font-mono text-[10px] tracking-[0.22em] text-ink-muted">{label}</span>
      <input
        name={name}
        type={type}
        required={required}
        ...
        className="mt-3 block w-full border-b border-line bg-transparent py-3 text-base font-light text-ink placeholder:text-ink-muted focus:border-ink focus:outline-none"
      />
    </label>
  );
}
  ...
  return (
    <label className="block">
      ...
      <select
        ...
        required={required}
        defaultValue=""
        className="mt-3 block w-full appearance-none border-b border-line bg-transparent py-3 pl-6 text-base font-light text-ink focus:border-ink focus:outline-none"
        style={{
          backgroundImage:
  ...
  return (
    <footer className="border-t border-line">
      <div className="mx-auto max-w-7xl px-6 py-12 text-center sm:px-10">
        ...
        <p className="mt-4 font-mono text-[11px] tracking-[0.18em] text-ink-muted">
